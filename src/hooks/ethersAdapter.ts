...
import { type WalletClient, useWalletClient, type PublicClient, usePublicClient } from 'wagmi'
...
export function publicClientToProvider(publicClient: PublicClient) {
  ...
  if (transport.type === 'fallback') {
    const providers = (transport.transports as ReturnType<HttpTransport>[]).map(({ value }) => new JsonRpcProvider(value?.url, network))
